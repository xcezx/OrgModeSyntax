<?php
class OrgModeSyntax_TreeRenderer
{
  protected $valueCallback, $isOrderedCallback;

  public function __construct($valueCallback, $isOrderedCallback = false)
  {
    $this->valueCallback = $valueCallback;
    $this->isOrderedCallback = $isOrderedCallback
      ? $isOrderedCallback
      : array($this, 'isOrderedDefaultCallback');
  }

  public function isOrderedDefaultCallback($node)
  {
    return true;
  }
  ...
  public function render(OrgModeSyntax_Tree_Root $root)
  {
  ...
  protected function renderNode($node)
  {
    ...
    if ($node->hasValue())
    {
      $ret .= $this->renderValue($node->getValue());
    }
